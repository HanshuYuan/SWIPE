...
data_new1=data_new1[rowSums(data_new1[,2:459])>9999,]
...
datanewP1=datanewP1[!grepl('Neg',datanewP1$name),]
```
...
COLORS = c("#00FFFF", "#000000", "#0000FF", "#FF00FF", "#778899", "#008000", "#800000", "#32CD32","#FF4509", "#808000", "#800080", "#ff0000", "#C0C0C0", "#008080", "#FF6347", "#FFFF00","#7CFC00", "#000080", "#FFD700", "#1E90FF", "#F778A1", "#A0522D", "#E0B0FF", "#493D26","#FFA62F","#0080FF","#FF4500","#8A2BE2",
...
ggsave("barPerson1_new_1.png",plot = pdf3,path = "~/OneDrive - UConn Health Center/SWIPE/output/",
       width = 15,height = 15,dpi = 320)

#write.csv(datanewP1_1,"~/OneDrive - UConn Health Center/SWIPE/P1.csv")
#write.csv(datanewP2_1,"~/OneDrive - UConn Health Center/SWIPE/P2.csv")
...
ggsave("barPerson2_new_1.png",plot = pdf3,path = "~/OneDrive - UConn Health Center/SWIPE/output/",
       width = 15,height = 15,dpi = 320)
```

```{r}
p1=read.csv("~/OneDrive - UConn Health Center/SWIPE/P1.csv",row.names = 1)
p2=read.csv("~/OneDrive - UConn Health Center/SWIPE/P2.csv",row.names = 1)
dp1=melt(p1)
dp2=melt(p2)
mean_taxa_per_group_p1 <- dp1 %>%
  group_by(Method, variable) %>%  # Group by 'Any' and 'variable' (taxon)
  summarise(mean_value = mean(value, na.rm = TRUE))

mean_taxa_per_group_p2 <- dp2 %>%
  group_by(Method, variable) %>%  # Group by 'Any' and 'variable' (taxon)
  summarise(mean_value = mean(value, na.rm = TRUE))
```

```{r}
pdf3=ggplot(mean_taxa_per_group_p1,aes(x=Method,y=mean_value,fill=variable))+
  geom_bar(stat = "identity")+
  ...
  theme(axis.title.x = element_text(size = 15,face = "bold"),
        #panel.background = element_blank(),
        axis.line = element_blank(),
        axis.text.x = element_text(size = 25,angle=0,face = "bold", vjust=1),
        axis.text.y = element_text(size = 25,face = "bold"),
        axis.title.y = element_text(size = 25,face = "bold"),#"legend.position = c(0.69, 0.78)"
        legend.text = element_text(size = 10,face = "bold"),
        legend.title = element_blank(),
        ...
        strip.text.y = element_text(size = 25,face = "bold"))+ylab("% Relative of Abundance")
pdf3
ggsave("barPerson1_mean.png",plot = pdf3,path = "~/OneDrive - UConn Health Center/SWIPE/output/",
       width = 15,height = 15,dpi = 320)
```


```{r}
pdf3=ggplot(mean_taxa_per_group_p2,aes(x=Method,y=mean_value,fill=variable))+
  geom_bar(stat = "identity")+
  #facet_grid(~Method, scales = "free_x")+ 
  scale_fill_manual(values=COLORS)+
  theme(axis.title.x = element_text(size = 15,face = "bold"),
        ...
        strip.text.y = element_text(size = 25,face = "bold"))+ylab("% Relative of Abundance")
pdf3
ggsave("barPerson2_mean.png",plot = pdf3,path = "~/OneDrive - UConn Health Center/SWIPE/output/",
       width = 15,height = 15,dpi = 320)
